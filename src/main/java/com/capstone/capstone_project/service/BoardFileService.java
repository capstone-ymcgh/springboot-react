...
import com.capstone.capstone_project.repository.BoardFileRepository;
...
public class BoardFileService {
    ...
    public List<RecBoardFileUploadDTO> upload(Long boardId, List<MultipartFile> multipartFiles) throws IOException {
        // 게시글 찾기
        RecBoardEntity board = boardRepository.findById(boardId).orElseThrow(
                () -> new ResourceNotFoundException("RecBoardEntity", "Board Id", String.valueOf(boardId))
        );
        List<BoardFileEntity> fileEntitys = new ArrayList<>();
        ...
        List<RecBoardFileUploadDTO> dtos = fileEntitys.stream().map(RecBoardFileUploadDTO::fromEntity).collect(Collectors.toList());

        return dtos;
    }

    public RecBoardFileDownDTO download(Long fileId) throws IOException {
        BoardFileEntity file = fileRepository.findById(fileId).orElseThrow(
        ...
        return RecBoardFileDownDTO.fromFileResource(file, contentType, content);
    }
